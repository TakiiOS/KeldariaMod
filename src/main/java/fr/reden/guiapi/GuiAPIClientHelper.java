...
public class GuiAPIClientHelper {
	 ...
	 */
	public static int getRelativeTextX(String text, int parentWidth, GuiConstants.HORIZONTAL_TEXT_ALIGNMENT horizontalTextAlignment)
	{
		switch (horizontalTextAlignment) {
			case CENTER:
				return (parentWidth - mc.fontRenderer.getStringWidth(text)) / 2;
			case RIGHT:
				return parentWidth - mc.fontRenderer.getStringWidth(text);
			default:
				return 0;
		}
	}
	 ...
	 */
	public static int getRelativeTextY(int lineIndex, int maxLines, int parentHeight, GuiConstants.VERTICAL_TEXT_ALIGNMENT verticalTextAlignment)
	{
		switch (verticalTextAlignment) {
			case CENTER:
				return (parentHeight - maxLines * mc.fontRenderer.FONT_HEIGHT) / 2 + lineIndex * mc.fontRenderer.FONT_HEIGHT;
			case BOTTOM:
				return parentHeight - maxLines * mc.fontRenderer.FONT_HEIGHT + lineIndex * mc.fontRenderer.FONT_HEIGHT;
			default:
				return lineIndex * mc.fontRenderer.FONT_HEIGHT;
		}
	}
	...
	public static void glScissor(int x, int y, int width, int height)
	{
		...
		GL11.glScissor(x * f, mc.displayHeight - (y + height) * f, MathHelper.clamp(width * f, 0, Integer.MAX_VALUE), MathHelper.clamp(height * f, 0, Integer.MAX_VALUE));
	}
	
	public static void drawBorderedRectangle(int left, int top, int right, int bottom, int borderSize, int backgroundColor, int borderColor)
	{
	...
	/**
	 * Basically just a copy of the vanilla method {@link GuiScreen#drawHoveringText(List, int, int, FontRenderer)}
	 */
