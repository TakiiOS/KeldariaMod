 ...
 */
package fr.nathanael2611.keldaria.mod.util.spy.imgur;
...
public class Uploader
{
    ...
    public static String upload(BufferedImage file)
    {
        HttpURLConnection conn = getHttpConnection(UPLOAD_API_URL);
        writeToConnection(conn, "image=" + toBase64(file));
        return getResponse(conn);
    ...
    public static String uploadAndGetLink(BufferedImage img)
    {
        String json = Uploader.upload(img);
        return new JsonParser().parse(json).getAsJsonObject().getAsJsonObject("data").get("link").getAsString();
    }
    ...
    private static String toBase64(BufferedImage file)
    {
        ...
        try
        {
            ...
            return URLEncoder.encode(Base64.getEncoder().encodeToString(os.toByteArray()), "UTF-8");
        }
