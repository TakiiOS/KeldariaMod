...
public class ModelLammergeier extends ModelBase
{
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        ...
        EntityLucrain lammergeier = (EntityLucrain) entityIn;
        ...
        for(int xx = (int) (entityIn.posX - (entityIn.width / 2)); xx < (entityIn.posX + (entityIn.width / 2));xx ++)
        {
            for(int zz = (int) (entityIn.posZ - (entityIn.width / 2)); zz < (entityIn.posZ + (entityIn.width / 2));zz ++)
            {
                BlockPos pos = new BlockPos(xx, entityIn.getPosition().getY() - 1, zz);
