...
public abstract class MixinContainer
{
    ...
    public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, EntityPlayer player)
    {
        ...
        Slot theSlot = slotId >= 0 && slotId <= inventorySlots.size() ? getSlot(slotId) : null;
        if(!InventoryHandler.canTouch(player, theSlot))
            return itemstack;
            ...
            if (!itemstack6.isEmpty() || !itemstack10.isEmpty())
            {
                if (itemstack6.isEmpty())
                {
                    if (slot4.canTakeStack(player))
                    {
                        ...
                        ((ISlotSwapAccessor)slot4).accessOnSwapCraft(itemstack10.getCount());
    ...
    protected void slotChangedCraftingGrid(World p_192389_1_, EntityPlayer p_192389_2_, InventoryCrafting p_192389_3_, InventoryCraftResult p_192389_4_)
    {
        if (!p_192389_1_.isRemote)
        {
            ...
            if(LocksHelper.isMatrixWillCraftKey(p_192389_3_))
            {
                itemstack = new ItemStack(p_192389_3_.getStackInSlot(1).getItem());
                itemstack.setTagCompound(p_192389_3_.getStackInSlot(0).getTagCompound().copy());
            }
            else
            {
                ...
                /*if(!SkillCrafts.getInstance().isPlayerCanCraft(entityplayermp, itemstack.getItem()))
                {
                    itemstack = ItemStack.EMPTY;
                }

                if(Keldaria.getInstance().getRegistry().getDisabledCrafts().isDisabled(itemstack.getItem()))
                {
                    itemstack = ItemStack.EMPTY;
                }*/
            }
